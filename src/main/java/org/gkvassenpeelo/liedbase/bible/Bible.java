...
import org.gkvassenpeelo.liedbase.liturgy.VerseRange;
...
public class Bible {
	...
	public static List<BiblePartFragment> getBiblePart(String translation, String book, int chapter, VerseRange verseRange) throws BibleException {
					...
					for (Character c : line.toCharArray()) {
						...
						if (prevCharType == CharType.number && getCharType(c) == CharType.character) {
							...
							if (!String.valueOf(c).equals(" ")) {
								...
								try {
									if (Integer.parseInt(sb.toString()) >= verseRange.getStartVerse()
											&& Integer.parseInt(sb.toString()) <= verseRange.getEndVerse()) {
										fragmentList.add(new BiblePartFragment(BiblePartFragment.DisplayType.superScript, sb.toString()));
										addVerse = true;
									}
									if (Integer.parseInt(sb.toString()) > verseRange.getEndVerse()) {
										s.close();
										return clean(fragmentList);
									}
