					...
					if (line.equals(verse)) {
						while (s.hasNextLine()) {
							...
							if (line.equals("" + (verseInteger + 1))
									|| readingNextSong(songIdentifier, songInteger, songNumberPostfix, nextSongNumberPostfix, line)) {
								s.close();
								return cleanup(songText);
							}
		...
		while (s.hasNextLine()) {
			...
			if (line.matches(String.format("^%s %s$", songIdentifier, songNumber))) {
				...
				while (s.hasNextLine()) {
					...
					// add the last verse/line, unless the line is the header of the next song
					if (!nextSong) {
						verses.add(verseText);
					}
					s.close();
